...
export default function FeedPage() {
  ...
  const fetchVotes = useCallback(async (after = 0) => {
    const url = after > 0 ? `/api/feed?after=${after}` : "/api/feed";
  ...
  return (
    <main className="min-h-screen flex flex-col items-center px-4 py-8">
      ...
      <div className="mb-6 text-center h-16 flex flex-col justify-center">
        ...
        <p className="font-mono-caps text-xs text-neutral-500">LIVE FEED</p>
      </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-1">
            {votes.map((vote) => (
              ...
              >
                {/* Winner */}
                <Link href={`/punk/${vote.winnerId}`} className="flex items-center gap-2 group">
                  <PunkImage punkId={vote.winnerId} className="w-8 h-8 rounded-full shrink-0" />
                  <div className="flex flex-col">
                    <span className="font-mono-caps text-[10px] text-green-400 group-hover:underline">
                      #{vote.winnerId.toString().padStart(4, "0")}
                    </span>
                ...
                {/* Loser */}
                <Link href={`/punk/${vote.loserId}`} className="flex items-center gap-2 group">
                  <PunkImage punkId={vote.loserId} className="w-8 h-8 rounded-full shrink-0" />
                  <div className="flex flex-col">
                    <span className="font-mono-caps text-[10px] text-red-400 group-hover:underline">
                      #{vote.loserId.toString().padStart(4, "0")}
                    </span>
                    <span className="font-mono-caps text-[9px] text-neutral-600">
                      {vote.loserElo}
                    </span>
